...
v0.5.0 内置 `ReadFile`、`WriteFile`、`EditFile`、`Bash`、`Glob`、`Grep`，另有按需发现的 `AskUser`。模型调用工具后，结果会自动回传，并在同一轮中继续执行或生成最终回答。
...
./mvnw test
./mvnw package
java -jar target/yuancode-v0.5.0.jar
```
...
```bash
java -jar target/yuancode-v0.5.0.jar --config ./config.yaml
```
...
本阶段不包含权限审批、MCP、Subagent、Worktree、并行工具调度或会话持久化。
